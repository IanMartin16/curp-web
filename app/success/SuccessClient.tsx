...
export default function SuccessClient({ sessionId }: { sessionId?: string }) {
  ...
  useEffect(() => {
    async function run() {
      if (!sessionId) {
        setErr("No llegó session_id. Revisa STRIPE_SUCCESS_URL con {CHECKOUT_SESSION_ID}.");
        setLoading(false);
        return;
      }

      try {
        const r = await fetch("/api/dashboard/session", {
          method: "POST",
        ...
        // ✅ revelar SOLO si el backend dice revealKey=true
        if (data.revealKey && data.apiKey) {
